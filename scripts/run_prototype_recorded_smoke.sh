...
export ROS_DOMAIN_ID="${HAND2ROBOT_ROS_DOMAIN_ID:-77}"
export ROS_LOCALHOST_ONLY=1
mkdir -p "$artifact_parent"
run_dir="$(mktemp -d "$artifact_parent/prototype_recorded.XXXXXX")"
...
setsid ros2 run hand_pipeline prototype_smoke_check --ros-args \
  -p required_records:=5 -p timeout_s:=12.0 >"$check_log" 2>&1 &
check_pid=$!
sleep 0.5

setsid ros2 launch hand_pipeline prototype_pipeline.launch.py \
  input_mode:=recorded enforce_capture_age:=false expected_steps:=5 \
  episode_id:=local-recorded-prototype-smoke \
...
if [ "$check_status" -ne 0 ]; then
  printf 'Recorded prototype check log:\n'
  tail -n 100 "$check_log"
  printf 'Recorded prototype launch log:\n'
...
python3 "$project_root/scripts/verify_episode_jsonl.py" "$episode_output" 5
sha256sum "$episode_output"
printf 'Recorded CPU prototype smoke passed. Artifact directory: %s\n' "$run_dir"
