...
| M3b Visual input and reconstruction | IN PROGRESS | Switchable camera/video RGB; live 21-joint input | Video frame smoke verified; MediaPipe/HaMeR adapter and camera hardware pending |
| M4 Local calibration and retargeting | VERIFIED | TF, scale, filtering, CPU safety target | Commits `8612de6`/`6c7cdbf`; 66 tests; three prototype smokes |
...
3. Connect MediaPipe or HaMeR once to `/visual/input/image_raw`; validate video mode first, then
   camera mode when hardware and approved dependencies are available.
